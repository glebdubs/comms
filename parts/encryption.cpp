...
CryptoManager::EVP_PKEY_ptr CryptoManager::loadPrivateKeyFromFile(const std::string& name, CryptoManager& manager) {
    // opening private key
    if(!doesThisFileExist("keys/"+name)) {
        std::cerr << "Place the PRIVATE key in a file called private.pem in /keys/ from this exec. Do not give it to anyone or distribute it. If anyone else may know it, generate a new public/private key pair.\n";
        std::exit(-1);
    }
...
CryptoManager::EVP_PKEY_ptr CryptoManager::loadPublicKeyFromFile(const std::string& name, CryptoManager& manager) {
    ...
    std::string plc;
    if(!doesThisFileExist("keys/"+name)) {
        std::cerr << "Place the PUBLIC key in a file called public.pem in /keys/ from this exec \n";
        std::exit(-1);
    }
